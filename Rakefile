...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/uxxman/tomo-plugin-goodjob/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("tomo-plugin-goodjob.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in tomo-plugin-goodjob.gemspec include the
...
namespace :bump do
  ...
  task :ruby do
    replace_in_file "tomo-plugin-goodjob.gemspec", /ruby_version = .*">= (.*)"/ => RubyVersions.lowest
